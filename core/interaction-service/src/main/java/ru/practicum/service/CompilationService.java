...
public class CompilationService {
    ...
    public List<CompilationDto> findCompilations(Boolean pinned, Pageable pageable) {
        ...
        // 4. Батч-загружаем события из event-service
        Map<Long, EventShortDto> eventsById = Collections.emptyMap();
        if (!allEventIds.isEmpty()) {
            List<EventShortDto> eventShortDtos = eventClient.getEvents(new ArrayList<>(allEventIds));
    ...
    public CompilationDto findCompilationById(Long compId) {
        ...
        if (!comp.getEventIds().isEmpty()) {
            List<EventShortDto> eventDtos = eventClient.getEvents(new ArrayList<>(comp.getEventIds()));
    ...
    public CompilationDto saveCompilation(NewCompilationDto newCompilationDto) {
        ...
        try {
            ...
            if (!savedCompilation.getEventIds().isEmpty()) {
                eventShortDtos = eventClient.getEvents(new ArrayList<>(savedCompilation.getEventIds()));
            }
    ...
    public CompilationDto updateCompilation(Long id, UpdateCompilationRequest updReqCompDto) {
        ...
        if (updReqCompDto.getEvents() != null) {
            if (!updReqCompDto.getEvents().isEmpty()) {
                ...
                if (!allPresent) {
                    ...
                    eventDtos = eventClient.getEvents(new ArrayList<>(newEventIds));
                }
            } else {
        ...
        try {
            ...
            if (eventDtos != null) {
                ...
                if (!comp.getEventIds().isEmpty()) {
                    currentEvents = eventClient.getEvents(new ArrayList<>(comp.getEventIds()));
                }
